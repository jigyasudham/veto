// Council orchestrator — runs all 5 agents in parallel, returns debate result
...
export async function runDebate(input: DebateInput): Promise<DebateResult> {
  ...
  // All 5 agents run in parallel — none depend on each other
  const [lead_dev, pm, architect, ux, devil] = await Promise.all([
  ...
  const votes = { lead_dev, pm, architect, ux, devil };
