// Compact tool surface — progressive disclosure for clients where 90 tool
...
export const META_TOOL_DEFINITIONS: ToolDef[] = [
  {
    ...
    description:
      'Searches the full Veto catalog (90 tools: agents, reviews, advisors, generators, memory, learning, git, workflow) by keyword and returns matching tool schemas. Compact mode exposes only core tools up front — call this first to discover the right tool, then invoke it via veto_call.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
