...
const EXT_RULES: Array<[(f: string, e: string) => boolean, string, string, string]> = [
...
function classify(file: string): { agent: string; tool: string; reason: string } {
  const ext = extname(file).toLowerCase();
  for (const [match, agent, tool, reason] of EXT_RULES) {
    if (match(file, ext)) return { agent, tool, reason };
  }
